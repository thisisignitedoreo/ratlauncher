...
def install():
	...
	if ui.radioButton_3.isChecked():
		mll.install.install_minecraft_version(install_version, minecraft_folder, callback=callback)
		app.processEvents()
		...
		if forge_version is None:
			...
			msg.setText("Ошибка!")
			msg.setInformativeText('К сожалению эта версия не поддерживает Forge!')
			msg.setWindowTitle("Упс!")
			msg.exec_()
			return
		mll.forge.install_forge_version(forge_version, minecraft_folder, callback=callback)
		app.processEvents()
		...
		try:
			...
			msg.setIcon(QMessageBox.Critical)
			msg.setText("Ошибка!")
			msg.setInformativeText('К сожалению эта версия не поддерживает Fabric!')
			msg.setWindowTitle("Упс!")
			msg.exec_()
			return
...
def run():
	...
	if ui.checkBox_2.isChecked():
		mll.install.install_minecraft_version(play_version, minecraft_folder, callback=callback)
	if ui.checkBox.isChecked():
		shutil.copytree("dependences/authlib", minecraft_folder + "/libraries/com/mojang/authlib", dirs_exist_ok=True)
	app.processEvents()
	ui.progressBar.setValue(0)
	options = {
	...
	minecraft_command = mll.command.get_minecraft_command(play_version, minecraft_folder, options)
	subprocess.call(minecraft_command)
	app.processEvents()

def fetchVersions():
	if os.path.isdir("gamefiles") == False:
		result = ["Нет установленых версий"]
	else:
		result = [i["id"] for i in mll.utils.get_installed_versions(minecraft_folder)]
	ui.comboBox.clear()
...
def addAccount():
	global accounts
	text, ok = QInputDialog.getText(form, 'Новый аккаунт', 
